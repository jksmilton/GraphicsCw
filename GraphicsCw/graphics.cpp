...
class Paths { 
	...
	void move(float timeDifference, int index){
		float change = timeDifference * speed / 100;
		glm::vec4 workingWith = Translate[index][3];

		if(workingWith[0] <  4.f && workingWith[1] >= -4.f){

			workingWith[0] += change;
			workingWith[1] = -4.f;

		} else if(workingWith[0] <= -4.f && workingWith[1] < 4.f){
...
int main(void){
	...
	if(!(sphereA.init() && coneB.init() && normalsC.init() && pathsE.init()))
		exit(EXIT_FAILURE);
